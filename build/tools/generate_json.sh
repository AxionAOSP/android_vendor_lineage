...
if [ -n "$OLD_TARGET_FILES" ]; then
    ...
    if [ -f "$NEW_TARGET_FILES" ]; then
        INCREMENTAL_ZIP="${FILENAME/-$ROMTYPE-/-INCREMENTAL-$ROMTYPE-}"
    ...
    # Automatically bootstrap baseline target-files for the next build!
    NEW_TARGET_FILES="$PRODUCT_OUT/obj/PACKAGING/target_files_intermediates/lineage_${TARGET_DEVICE}-target_files.zip"
    if [ -f "$NEW_TARGET_FILES" ]; then
        mkdir -p "$ARCHIVE_DIR"
        ARCHIVED_BASELINE="$ARCHIVE_DIR/${FILENAME%.zip}-target_files.zip"
        echo "No previous baseline found. Archiving target-files for future incremental builds..."
        cp "$NEW_TARGET_FILES" "$ARCHIVED_BASELINE"
    fi
fi
